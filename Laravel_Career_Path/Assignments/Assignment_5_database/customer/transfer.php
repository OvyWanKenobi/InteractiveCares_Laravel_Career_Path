...
if (!isset($_SESSION['customer_uniqueid'])) {
...
$customer_id = $_SESSION["customer_uniqueid"];

$account = new AccountInfo();
$accountInfo = $account->getAccountInfo('unique_id', $customer_id);
// var_dump($accountInfo);
$customer_name = $accountInfo['name'];
$customer_email = $accountInfo['email'];
$name_parts = explode(' ', $customer_name);


$transactions = new Transaction($customer_id);

$balance = $transactions->getCustomerBalance();
$customer_balance = number_format($balance, 2);

$errorBag = [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $amount = sanitize($_POST['amount']);
  $receiver_email = sanitize($_POST['email']);

  $transfer = new Transfer($transactions, $customer_id);

  if ($transfer->transfer($receiver_email, $amount)) {
    flash('success', 'Amount $' . $amount . ' has been Transfered to ' . $receiver_email);
    header('Location: transfer.php');
    exit;
  } else {
    $errorBag = $transfer->errorBag;
  }
}
                ...
                <div>
                  <button
                    ...
                    <!-- <img
                        ...
                        alt="<?= $customer_name ?> Hasan Shaon" /> -->
                    <span
                      class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                      <span class="font-medium leading-none text-emerald-700"><?= $name_parts[0][0] . $name_parts[1][0] ?></span>
                    </span>
                  </button>
                </div>
    ...
    <main class="-mt-32">
      <div class="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg p-2">
          ...
          if ($success_message) :
          ?>
            <div class="mt-2 mb-2  bg-teal-100 border border-teal-200 text-sm text-teal-800 rounded-lg p-3 dark:bg-teal-800/10 dark:border-teal-900 dark:text-teal-500 text-center" role="alert">
          ...
          if ($error_message) :
          ?>
            <div class="mt-2 mb-2  bg-red-100 border border-red-200 text-sm text-red-800 rounded-lg p-3 dark:bg-red-800/10 dark:border-red-900 dark:text-red-500 text-center" role="alert">
            ...
            <div class="px-4 py-5 sm:p-6">
              <div class="mt-4 text-sm text-gray-500">
                <form
                  ...
                  <?php if (isset($errorBag['amount'])) : ?>
                    <p class="text-xs text-red-600 mt-1"> * <?= $errorBag['amount'] ?></p>
                  <?php endif; ?>
